...
export const createSeedData = async () => {
  try {
    ...
    const officeCategory = categories.find(
      (c) => c.name === "Oficina Privada"
    )?.id;
    const meetingCategory = categories.find(
      (c) => c.name === "Sala de Reuniones"
    )?.id;
    const coworkingCategory = categories.find(
      (c) => c.name === "Coworking"
    )?.id;
    ...
    const sampleSpaces = [
      {
        ...
        description:
          "Elegante oficina privada perfecta para equipos pequeños. Ubicada en una zona comercial con excelente conectividad.",
        ...
        policies:
          "No se permite fumar. Política de cancelación: 24 horas de anticipación.",
        owner_id: user.id,
      ...
      },
      {
        ...
        description:
          "Sala de reuniones equipada con tecnología de punta para presentaciones profesionales.",
        address: "Calle Comercio 456",
        city: "Ciudad de México",
        price_per_hour: 480.0,
        capacity: 10,
        area_sqm: 25,
