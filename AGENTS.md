...
Keysely Platform is a **flexible workspace marketplace** built with React 18, TypeScript, and modern backend services. The application connects users with workspaces (offices, meeting rooms, coworking spaces) in Mexico, featuring real-time chat, Stripe payments, and bilingual support (Spanish/English).

**Tech Stack:**

- Frontend: React 18 + TypeScript + Vite + Tailwind CSS + shadcn/ui
- Backend: Cloud Services (Auth, PostgreSQL, Storage, Realtime)
...
cp .env.example .env
# Edit .env with your Backend, Stripe, and Contentful credentials
...
- Backend service account with configured project
...
All database operations go through the backend client:
...
- Server secrets go in Edge Functions, not client code

### Backend Security

- **Always use RLS policies** - never disable Row Level Security
- **Validate user input** on both client and server
- **Use parameterized queries** (Client does this by default)
- **Verify ownership** before updates/deletes

### Authentication

- Use built-in Auth - never roll your own
...
3. Update TypeScript types (auto-generated by Backend CLI)
...
- Check Backend project is active
...
- **Primary DB**: Used for basic listing and simple filtering when no search term is present.
...
1. **Orchestrator (`Explore.tsx`)**:
   ...
   - Fetches data using `useSpaces` (Primary DB) or `useTypesenseSearch` (Typesense).
...
- **Filtering**:
  ...
  - **Primary DB**: (Currently limited or relies on client-side filtering for complex geo-queries, though simple city filtering is supported).
